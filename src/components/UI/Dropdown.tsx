...
interface IProps {
  ...
  onMultipleSelect?: (option: Set<any>) => void;
...
export const Dropdown: React.FC<IProps> = ({
  ...
  const isSelected = (id: number) => {
    return [...selectedItems].map((item) => item.id).includes(id);
  };

  const toggleItem = (index: number) => {
    const option = optionList.find((option) => option.id === index);
    const selectedItemsCopy = new Set(selectedItems);

    if (isSelected(index)) {
      selectedItemsCopy.delete(option);
    } else {
      selectedItemsCopy.add(option);
    }
    setSelectedItems(selectedItemsCopy);

    const selectedValues = [...selectedItemsCopy].map((item) => item[valueKey]);
    onChange && onChange(new Set(selectedValues));
    onMultipleSelect && onMultipleSelect(selectedItemsCopy);

    if (selectedItemsCopy.size === 0) {
      setIsOpen(false);
    }
  };
  ...
  const handleItemClick = (index: number) => {
    if (multiple) {
      ...
      onChange && option && onChange(option[valueKey] || null);
    }
  };
      ...
      >
          ...
          className={classNames(
            classes[
              `dropdown__header-title${
                selectedItemIndex !== -1 || selectedItems.size !== 0 ? '-active' : ''
              }`
            ],
            classes[classname]
          )}
        >
          {!multiple
            ...
            : selectedItems.size
            ? [...selectedItems]
                .slice(0, selectedItems.size > 3 ? 3 : selectedItems.size)
                .map((item) => item.name)
                .join(', ')
            : placeholder}
        </div>
                  ...
                  .map((option) => (
                    <li
                      className={classNames(
                        classes[
                          `dropdown__body-item${
                            selectedItemIndex === option.id || isSelected(option.id)
                              ? '--active'
                              : ''
                          }`
                        ],
